...
"""
Configuration for tests! There are a whole list of hooks you can define in this
file to run before, after, or to mutate how tests run. Commonly for most of our
work, we use this file to define top level fixtures that may be needed for
tests throughout multiple test files.

In this case, while we aren't using this fixture in our tests, the prime use
case for something like this would be when we want to preload a file to be used
in multiple tests. File reading can take time, so instead of re-reading the
file for each test, read the file once then use the loaded content.
...
def mock_mqtt_client(mocker):
    ...
    mock_wait_for_publish = mocker.patch.object(MQTTMessageInfo,
                                                "wait_for_publish")
    ...
    mock_mqtt_client.subscribe = mocker.MagicMock(
        return_value=mqtt_message_info)
...
def mock_time(mocker):
    return mocker.patch("time.time", return_value=1001)
