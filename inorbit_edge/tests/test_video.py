...
def test_robot_session_register_camera(
    ...
    #  OpenCV exception "error: (-215:Assertion failed) !_filename.empty() in
    #  function 'open'" This is fine for the purpose of this test that is
    #  verify that the Capture and Camera stream threads stop when the robot
    #  session disconnects.
    ...
    stop_cameras_streaming_spy = mocker.spy(
        robot_session,
        "_stop_cameras_streaming",
    )
    ...
    # Override _is_disconnected method to simulate MQTT disconnection
